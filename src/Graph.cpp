...
// Functions
unsigned int** Graph::VEtoAdjMatrix(unsigned int V, set<pair<char,char>> E) {
    unsigned int **matrix = (unsigned int**) malloc(sizeof(unsigned int*) * this->getnVertices());
    if(matrix == NULL) return NULL;

    for(unsigned int i = 0; i < V; i++) {
        matrix[i] = (unsigned int*) malloc(sizeof(unsigned int) * this->getnVertices());
    ...
    for(unsigned int i = 0; i < this->getnVertices(); i++) {
        ...
        // Percorrendo todo o 'E' e anotado suas adjacências
    set<pair<char,char>>::iterator it;
    for(it = E.begin(); it != E.end(); it++) {
        // converte de char para int e desloca o char em 'a' menos.
        // std::cout << "First: " << (*it).first - 'a' << " Second: " << (*it).second - 'a' << std::endl;
...
// Setter
void Graph::setAdj(unsigned int **adj) {this->Adj = adj;}
void Graph::setnVertices(unsigned int n) {this->nVertices = n;}
void Graph::setnEdges(unsigned int n) {this->nEdges = n;}

// Getter
unsigned int** Graph::getAdj() {return this->Adj;}
unsigned int Graph::getAdj(unsigned int i, unsigned int j) {return this->Adj[i][j];}
